...
async def ban(ctx, member: discord.Member, *, reason: str = "사유 없음"):
    try:
        ...
        if log_channel:
            ...
            embed.add_field(name="밴 대상", value=member.mention, inline=True)
...
async def 청소(ctx, amount: int = 5):
    ...
    if log_channel:
        ...
        embed.add_field(name="삭제 수", value=str(len(deleted)-1), inline=True)
...
async def role(ctx, member: discord.Member, *, role_input: str):
    ...
    try:
        ...
        if log_channel:
            ...
            embed.add_field(name="부여된 역할", value=role.name, inline=True)
            embed.set_footer(text=f"처리자: {ctx.author}", icon_url=ctx.author.avatar.url if ctx.author.avatar else None)
            await log_channel.send(embed=embed)
...
@bot.command()
@commands.has_permissions(moderate_members=True)  # 타임아웃 권한이 필요합니다.
